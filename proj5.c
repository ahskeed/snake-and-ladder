...
void numbers(int xcen[2],int ycen[2])
{
	...
	for(i=0;i<10;i++)
	{
		...
		for(j=0;j<10;j++)
		{
			...
			if(i==0&&j==0)
			{
				scanfill(0.0,0.0,400.0,0.0,400.0,400.0,0.0,400.0);
			}
			else
			{
				glBegin(GL_POLYGON);
...
void segment()
{
	...
	s=fopen("seg.dat","rb");
	while(fread(&table[i],sizeof(seg),1,s))
		i++;
	fclose(s);
}

// Update the trop file with new mary position 
void file_update()
{
	FILE *f,*f1;
	int count=0;
	struct disp d1;
	f=fopen("trop.dat","rb");
	f1=fopen("temp.dat","w+b");
	while(fread(&d1,sizeof(d1),1,f))
	{
		count++;
		if(count<=table[0].size)
		{
			fwrite(&d1,sizeof(d1),1,f1);
		}
		else
		{
			switch(count-table[0].size)
			{
				case 1: d1.x=mptx[0];
					d1.y=mpty[0];
					break;
...
void Scale(float sx , float sy)
{
	...
	drawstring(1000,2500,0,"WELCOME TO SNAKES AND LADDERS!!");
	glColor3f(0,1,1);
	matrixSetIdentity(m);
	m[0][0] = sx;
	m[0][2] = (1-sx)*refpt[0];
